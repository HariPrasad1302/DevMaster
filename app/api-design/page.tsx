...
let apiTopics: any[] = [];
try {
  const mod = require('@/data/apiroadmap');
  apiCategories = mod.apiCategories ?? [];
  apiTopics = mod.apiTopics ?? [];
} catch {}
...
export default function ApiDesignPage() {
  ...
  const filtered = apiTopics.filter((t: any) =>
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '1100px', margin: '0 auto' }}>
      ...
      <p style={{ color: 'var(--text-secondary)', marginBottom: '1.75rem', fontSize: '0.9rem' }}>
        From HTTP fundamentals to gRPC, OAuth2, GraphQL, caching & real-time APIs. Click any topic to expand.
      </p>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
        {filtered.map((topic: any) => {
          const cat = apiCategories.find((c: any) => c.id === topic.category);
      ...
      {filtered.length === 0 && (
        <div style={{ textAlign: 'center', padding: '4rem 2rem', color: 'var(--text-muted)' }}>
          Data loading... Make sure apiroadmap.ts is generated.
        </div>
      )}
    </div>
  );
