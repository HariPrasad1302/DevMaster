...
import { roadmapTopics, roadmapCategories } from '@/data/roadmap';

export default function PythonRoadmapPage() {
  const [selectedCategory, setSelectedCategory] = useState<string>('all');
  const [expandedId, setExpandedId] = useState<string | null>(null);

  const filtered = roadmapTopics.filter(t =>
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '1100px', margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', fontWeight: 800, letterSpacing: '-0.03em', marginBottom: '0.4rem' }}>
        Python → Senior Dev Roadmap
      </h1>
      <p style={{ color: 'var(--text-secondary)', marginBottom: '1.75rem', fontSize: '0.9rem' }}>
        Complete learning path from Python basics to Django & FastAPI. Click any topic to expand.
      </p>
        ...
        {roadmapCategories.map(cat => (
          <button
            ...
            style={{
              ...
              cursor: 'pointer', borderRadius: '8px', border: selectedCategory === cat.id ? 'none' : '1px solid var(--border)',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
        {filtered.map((topic) => {
          const cat = roadmapCategories.find(c => c.id === topic.category);
          ...
          return (
            <div key={topic.id} className="card" style={{ overflow: 'hidden' }}>
              ...
              >
                <div style={{ width: '36px', height: '36px', borderRadius: '8px', background: `${cat?.color}18`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.1rem', flexShrink: 0 }}>
                  {cat?.icon}
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexShrink: 0 }}>
                  ...
                  <span style={{ color: 'var(--text-muted)', fontSize: '0.85rem', transform: isExpanded ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }}>▾</span>
                </div>
              </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
                    ...
                    {topic.codeExample && (
                      <div>
                        ...
                        <pre style={{ fontSize: '0.78rem', lineHeight: 1.6, maxHeight: '280px', overflowY: 'auto' }}>
